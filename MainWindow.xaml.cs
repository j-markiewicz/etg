...
namespace etg
{
    public partial class MainWindow : Window
    {
        ...
        {
            DiagnosticsText.Text = "Wczytaj plik i kliknij 'Otwórz', aby zobaczyć diagnostykę.";
        ...
        private void RunDiagnostics(Graph graph)
        {
            ...
            var specializedCount = graph.Procs.Count(p => p.Specialized == 1);
            var generalCount = graph.Procs.Count(p => p.Specialized == 0);
            bool hasWarnings = false;

            // Sprawdzenie: zadania DT/CDT bez procesora specjalistycznego
            var dtTasks = graph.Tasks.Where(t => t.TaskType == TaskType.DT || t.TaskType == TaskType.CDT).ToList();
